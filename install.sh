...
instalar_fof() {
    ...
    # Criar link para o modo normal
    ln -sf "$INSTALL_DIR/iniciar_fof.sh" "$BIN_DIR/fof"
    chmod +x "$INSTALL_DIR/iniciar_fof.sh"
    chmod +x "$BIN_DIR/fof"

    # Criar link para o modo compatibilidade
    if [ -f "$INSTALL_DIR/iniciar_fof_compat.sh" ]; then
...
EOF
        ...
        print_info "Use este atalho se tiver problemas com aceleração gráfica"
    fi
...
Após a instalação:
  ...
  - Dois atalhos serão criados no menu de aplicativos:
    * Fedora Only Fans (modo normal)
    * Fedora Only Fans (Modo Compatibilidade)
