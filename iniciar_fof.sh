...
# abrir. Estratégia em camadas:
#
# 1. KDE: `kstart --iconify` faz o terminal abrir já minimizado.
#    Requer o pacote kde-cli-tools (normalmente já instalado no KDE).
# 2. Fallback universal: `xdotool` ou `wmctrl` minimizam a janela
#    pelo título após ela aparecer.
# 3. Se nada funcionar, o terminal abre normal (comportamento antigo).

_minimizar_janela_bg() {
    local titulo="$1"

    # Roda desanexado para sobreviver ao exec do terminal.
    (
        # Tenta por até 6s — o terminal pode demorar pra abrir.
        local tentativas=0
        while [ $tentativas -lt 30 ]; do
            sleep 0.2
            tentativas=$((tentativas + 1))

            # xdotool: busca por nome da janela e minimiza
            ...
            # wmctrl: busca por título e esconde (equivale a minimizar)
...
abrir_no_terminal_nativo() {
    ...
    # ─── KDE: kstart --iconify (abre já minimizado) ────────────
    if [ "$NO_MINIMIZE" != true ] && command -v kstart &> /dev/null; then
        if command -v konsole &> /dev/null; then
            log_debug "Usando kstart --iconify + konsole (KDE, minimizado)"
            exec kstart --iconify konsole --title "$titulo" -e bash "$script_path" --no-fork
        fi
        if command -v ptyxis &> /dev/null; then
            log_debug "Usando kstart --iconify + ptyxis (KDE, minimizado)"
            exec kstart --iconify ptyxis --title "$titulo" -- bash "$script_path" --no-fork
        fi
    fi

    # ─── Fallback: abrir e minimizar via xdotool/wmctrl ────────
    local tentar_minimizar=false
    if [ "$NO_MINIMIZE" != true ]; then
        if command -v xdotool &> /dev/null || command -v wmctrl &> /dev/null; then
            tentar_minimizar=true
        fi
    fi

    # xdg-terminal-exec (mais genérico)
    if command -v xdg-terminal-exec &> /dev/null; then
        log_debug "Usando xdg-terminal-exec"
        [ "$tentar_minimizar" = true ] && _minimizar_janela_bg "$titulo"
...
quando possível (KDE via kstart, outros via xdotool/wmctrl).
